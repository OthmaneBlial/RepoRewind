...
describe('repository analysis integration', () => {
  it('reads actual commits, tags, contributors, renames, and additions', async () => {
    ...
    expect(history.commits[1].files).toContainEqual(expect.objectContaining({
      path: 'src/city.ts',
    ...
    expect(limited.commits[0].files.map((file) => file.path).sort()).toEqual([
      'src/city.ts', 'src/traveler.ts',
    ])
  })
  ...
  it('replays a truthful first-parent city and records merge topology', () => {
    ...
    expect(history.commits.at(-1)?.files).toContainEqual(expect.objectContaining({
      path: 'src/traveler.ts', status: 'added', additions: 1,
    }))
    expect(history.branches?.map((branch) => branch.name)).toEqual(expect.arrayContaining(['main', 'feature/travelers']))
